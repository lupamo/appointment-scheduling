...
CREATE TABLE bookings (
	...
	status TEXT NOT NULL DEFAULT 'pending_payment'
		CHECK (status IN ('pending_payment', 'confirmed', 'expired', 'cancelled', 'no_show', 'completed', 'rescheduled')),
...
-- Prevents double-booking / overlapping bookings for the same business
-- while a slot is pending payment or confirmed. Subsumes a plain unique
-- index on slot_start since it also catches partial overlaps across
-- services of different durations.
...
CREATE TABLE payment_events (
	...
	event_type TEXT NOT NULL,  -- stk_initiated | callback_success | callback_failed | timeout | manual_status_check
	raw_payload JSONB,
	created_at TIMESTAMPTZ DEFAULT now()
);
